...
<a id="${subject.subjectId}" title="${subject.subjectName }" href="javascript:void(0)" onclick="submitForm(${subject.subjectId})">${subject.subjectName }</a>
      ...
      <section class="c-sort-box unBr">
        ...
        <div>
          <div class="paging">
            <!-- undisable这个class是否存在，取决于数据属性hasPrevious，prevent是阻止标签默认行为
            对于a标签阻止其超链接跳转，执行我们设置的方法，对于这个是gotoPage()方法 -->
            <a
...
export default {
  //前端的另一种调用方式，异步调用，在页面组件加载之前调用，只调用一次
  //params：相当于之前的this.$route.params，取出路径参数，等价params.id，注意params.后面的要和_id.vue中_后面的名字一致
  //error：错误信息
  asyncData({ params, error }) {
    //注意这里的return 不要回车
    return teacherApi.getPageList(1, 8).then(response => {
      //将后台数据进行赋值，这里是一种简写方式，这样写之后就直接有了data这个数据，我们不需要再写data并在里面定义data
